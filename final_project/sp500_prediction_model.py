...
@task(name = 'loading market data',  retries=3, retry_delay_seconds=5)
def load_data():
    try:
        ...
        s3.put_object(Bucket=S3_BUCKET, Key='sp500_data.csv', Body=csv_buffer)
        return sp500
...
@task(name = 'making preficitons', log_prints = True)
...
@task(name = 'backtesting model', log_prints =  True)
...
if __name__ == "__main__":
    main_flow().register(project_name="sp500 prediction")